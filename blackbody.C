...
void disp(void)
{
  ...
  for (double x=-0.9; x<0.9; x+=1.8/window_size)
  {
   ...
   glEnd();
   glColor4f(1,1,1,0.6);
    for (double l=lmin; l<lmax; l+=(lmax-lmin)/(window_size*8))
    {
      double x=getx(l), y=ampl(l,t)/dmaxamp-0.8;
     circle(x,y,0.01,5);
    }      
  ...
  glEnd();
    ...
    {
    lastx=x;
    renderBitmapString(x,-0.90,0,GLUT_BITMAP_HELVETICA_18,thestring);
    }
  } 
  lastx=-10;
  ...
  glColor4f(1,1,1,1);
    ...
    {
    lastx=x;
    renderBitmapString(x,0.87,0,GLUT_BITMAP_HELVETICA_18,thestring);
    }
  } 
  sprintf(thestring,"Energy per photon (eV)");
  renderBitmapString(0,0.92,0,GLUT_BITMAP_HELVETICA_18,thestring);
  sprintf(thestring,"Wavelength (nanometers)");
  renderBitmapString(0,-0.95,0,GLUT_BITMAP_HELVETICA_18,thestring);
  sprintf(thestring,"Temp = %d Kelvin (%d Fahrenheit)",(int)t,(int)((t-273)*1.8+32));
  renderBitmapString(0,0.5,0,GLUT_BITMAP_HELVETICA_18,thestring);
...
int main(int argc, char **argv)
{
  ...
  printf("\t-/=     : change temperature (100K at a time)\n");
