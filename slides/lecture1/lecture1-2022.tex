\documentclass[10pt]{beamer}
\usepackage{amsmath}
\usepackage{mathtools}
\usepackage{multimedia}
\usepackage{hyperref}
\usefonttheme{professionalfonts} % using non standard fonts for beamer
\usefonttheme{serif} % default family is serif
%\documentclass[12pt]{beamerthemeSam.sty}
\usepackage{epsf}
%\usepackage{pstricks}
%\usepackage[orientation=portrait,size=A4]{beamerposter}
\geometry{paperwidth=200mm,paperheight=125mm}
%DT favorite definitions
\def\LL{\left\langle}	% left angle bracket
\def\RR{\right\rangle}	% right angle bracket
\def\LP{\left(}		% left parenthesis
\def\RP{\right)}	% right parenthesis
\def\LB{\left\{}	% left curly bracket
\def\RB{\right\}}	% right curly bracket
\def\PAR#1#2{ {{\partial #1}\over{\partial #2}} }
\def\PARTWO#1#2{ {{\partial^2 #1}\over{\partial #2}^2} }
\def\PARTWOMIX#1#2#3{ {{\partial^2 #1}\over{\partial #2 \partial #3}} }

\def\rightpartial{{\overrightarrow\partial}}
\def\leftpartial{{\overleftarrow\partial}}
\def\diffpartial{\buildrel\leftrightarrow\over\partial}

\def\BS{\bigskip}
\def\BC{\begin{center}}
\def\EC{\end{center}}
\def\BCC{\begin{columns}}
\def\ECC{\end{columns}}
\def\HC{\column{0.5\textwidth}}
\def\BI{\begin{itemize}}
\def\EI{\end{itemize}}
\def\BE{\begin{displaymath}}
\def\EE{\end{displaymath}}
\def\BEA{\begin{eqnarray*}}
\def\EEA{\end{eqnarray*}}
\def\BNEA{\begin{eqnarray}}
\def\ENEA{\end{eqnarray}}
\def\EL{\nonumber\\}


\newcommand{\etal}{{\it et al.}}
\newcommand{\gbeta}{6/g^2}
\newcommand{\la}[1]{\label{#1}}
\newcommand{\ie}{{\em i.e.\ }}
\newcommand{\eg}{{\em e.\,g.\ }}
\newcommand{\cf}{cf.\ }
\newcommand{\etc}{etc.\ }
\newcommand{\atantwo}{{\rm atan2}}
\newcommand{\Tr}{{\rm Tr}}
\newcommand{\dt}{\Delta t}
\newcommand{\op}{{\cal O}}
\newcommand{\msbar}{{\overline{\rm MS}}}
\def\chpt{\raise0.4ex\hbox{$\chi$}PT}
\def\schpt{S\raise0.4ex\hbox{$\chi$}PT}
\def\MeV{{\rm Me\!V}}
\def\GeV{{\rm Ge\!V}}

%AB: my color definitions
%\definecolor{mygarnet}{rgb}{0.445,0.184,0.215}
%\definecolor{mygold}{rgb}{0.848,0.848,0.098}
%\definecolor{myg2g}{rgb}{0.647,0.316,0.157}
\definecolor{A}{rgb}{1.0,0.3,0.3}
\definecolor{B}{rgb}{0.0,1.0,0.0}
\definecolor{C}{rgb}{1.0,1.0,0.0}
\definecolor{D}{rgb}{0.5,0.5,1.0}
\definecolor{E}{rgb}{0.7,0.7,0.7}
\definecolor{abtitlecolor}{rgb}{1.0,1.0,1.0}
\definecolor{absecondarycolor}{rgb}{0.0,0.416,0.804}
\definecolor{abprimarycolor}{rgb}{1.0,0.686,0.0}
\definecolor{Red}           {cmyk}{0,1,1,0}
\definecolor{Grey}          {cmyk}{.7,.7,.7,0}
\definecolor{Blue}          {cmyk}{1,1,0,0}
\definecolor{Green}         {cmyk}{1,0,1,0}
\definecolor{Brown}         {cmyk}{0,0.81,1,0.60}
\definecolor{Silver}        {rgb}{0.95,0.9,1.0}
\definecolor{Sky}           {rgb}{0.07,0.0,0.2}
\definecolor{Darkbrown}     {rgb}{0.4,0.3,0.2}
\definecolor{40Gray}        {rgb}{0.4,0.4,0.5}
\usetheme{Madrid}


\setbeamercolor{normal text}{fg=Silver,bg=Sky}

%AB: redefinition of beamer colors
%\setbeamercolor{palette tertiary}{fg=white,bg=mygarnet}
%\setbeamercolor{palette secondary}{fg=white,bg=myg2g}
%\setbeamercolor{palette primary}{fg=black,bg=mygold}
\setbeamercolor{title}{fg=abtitlecolor}
\setbeamercolor{frametitle}{fg=abtitlecolor}
\setbeamercolor{palette tertiary}{fg=white,bg=Darkbrown}
\setbeamercolor{palette secondary}{fg=white,bg=absecondarycolor}
\setbeamercolor{palette primary}{fg=white,bg=40Gray}
\setbeamercolor{structure}{fg=abtitlecolor}

\setbeamerfont{section in toc}{series=\bfseries}

%AB: remove navigation icons
\beamertemplatenavigationsymbolsempty
\title[Introduction to Astronomy]{
  \textbf {What's out there... \\ ... how we learn about it ... \\ ... and why it's awesome. \\}
%\centerline{}
%\centering
%\vspace{-0.0in}
%\includegraphics[width=0.3\textwidth]{propvalues_0093.pdf}
%\vspace{-0.3in}\\
%\label{intrograph}
}

\author [Astronomy 101]{Astronomy 101\\Syracuse University, Fall 2022\\Walter Freeman}

\date{\today}

\begin{document}

\frame{\titlepage}

\frame{
	
%	\BCC
%	\HC
%	\BC
%	\Large
%	Course Discord server:
%	
%	\BS
%	\includegraphics[width=0.7\textwidth]{discord-qr-code.png}
%	
%	\large
%	
%	
%	{\tt https://discord.gg/9DkbcEXjtY}
%	
%	\EC
%	\HC
	\BC
	\Large
	Course website:
	
	\BS
	\includegraphics[width=0.4\textwidth]{website-qr-code.png}
	
	\Large
	{\tt https://walterfreeman.github.io/ast101/}
	
	
	
	

	
	
	\medskip
	\it 
	\it We will post materials for this class here rather than Blackboard. \\Blackboard will only be used to post grades.
	\EC
%	\ECC
%	
%	\BCC
%	
%	\HC
%	\BC
%	\small
%	\begin{minipage}{0.8\textwidth}
%	\it (You don't have to join the Discord server, but it's a place where you can have fun with your colleagues and instructors and ask questions!)
%	\end{minipage}
%	\EC
%	\HC
%	\BC
%	\small
%		\begin{minipage}{0.8\textwidth}
%	\it We will post materials for this class here rather than Blackboard. Blackboard will only be used to post grades.
%	\end{minipage}
%
%	\EC
%	\ECC

}


\frame{\frametitle{\textbf{Welcome!}}
\large

The size and age of the Cosmos are beyond ordinary human understanding. 
Lost somewhere between immensity and eternity is our tiny planetary home. 
In a cosmic perspective, most human concerns seem insignificant, even petty. 
And yet our species is young and curious and brave and shows much promise. 

\BS

\BCC
\column{0.4\textwidth}
\BC
\includegraphics[width=0.9\textwidth]{blue-marble.jpg}\\
\scriptsize

\it ``The Blue Marble'': Earth photographed from space by the crew of {\rm Apollo 17}, 29,000 km from Earth.

\EC
\column{0.6\textwidth}
In the last few millennia we have made the most astonishing and unexpected discoveries about the Cosmos and our place within it, 
explorations that are exhilarating to consider. 
They remind us that humans have evolved to wonder, that understanding is a joy, 
that knowledge is prerequisite to survival. 

\BS

I believe our future depends powerfully on how well we understand this Cosmos in which we float like a mote of dust in the morning sky.
\BS\BS

\begin{flushright}--Carl Sagan, American astronomer, from {\it Cosmos} (1980)\end{flushright}

\vspace{0.2in}{}

\ECC

}

\frame{\frametitle{\textbf{Welcome!}}
\large

\begin{minipage}{0.5\textwidth}
When we contemplate the whole globe as one great dewdrop, striped and dotted with continents and islands, flying through space with other stars all singing and shining together as one, the whole universe appears as an infinite storm of beauty.

\begin{flushright}--John Muir, from {\it Travels in Alaska} (1915) \end{flushright}

\BC
\includegraphics[width=0.9\textwidth]{chiricahua-small.jpg}\\
\it \scriptsize Chiricahua National Monument, Arizona, before sunrise: 15 seconds,  14mm f/2.5, ISO 3200
\EC

\end{minipage}
\begin{minipage}{0.49\textwidth}
	\BC
\includegraphics[height=0.85\textheight]{milky-way-colorado.jpg}\\
\scriptsize
\it The Milky Way seen from outside Silverton, Colorado (3300m elevation). \\13 second exposure, 18mm f/1.8, ISO 1600
\EC
\end{minipage}

	}




\frame{\frametitle{\textbf{Welcome!}}
\Large
Today:

\begin{itemize}
\item{Who we are}
\item{Who you are}
\item{What this class will be}
\pause
\item{What the Universe is, too}

\small \it (and how we measure it)

\end{itemize}
}

\frame{\frametitle{\textbf{Introduction}}
\Large
Course website: \url{https://walterfreeman.github.io/ast101/}
\BS

\color{A}
Professor: Walter Freeman (he/him)

\BCC
\HC
\begin{center}
	\color{B}
	Graduate student instructors:
\end{center}
\HC
\begin{center}
		\color{C}
	Undergraduate coaches:
\end{center}
\ECC

\BCC
\HC
	\color{B}
	\begin{itemize}
			\color{B}
		\item Nada Nabil Elsayed Elmeigy
	\item Keisi Kacanja
	\item  Chandler Martin
	\item  Chad Skerbec
	\item  Byron Sleight
	\item  Sierra Thomas
	\item  Dylan Van Allen
	\item  Lindsay Wilson
\end{itemize}
\HC
	\color{C}
\BI
	\color{C}
\item Juanitta ``AJ'' Bekoe
\item Kiersten Edwards
\item Sydney Jud
\item Dominic Naggar
\item Chloe Britton Naime
\item Mykaylo Rafalskyy
\EI
\ECC

\bigskip
\bigskip

}

%\frame{\frametitle{\textbf{Course / department Discord server}}
%\Large
%We set up a Discord server during the pandemic to help students find community while stuck at home during the pandemic.
%
%It was a great success, so we want to keep the tradition alive.
%
%\BS
%
%Using it isn't required for class, but we hope it'll be a helpful resource (and fun place) for people who want to use it.
%
%\BS
%
%Take a few minutes to join now, say hi, and send memes...
%}



\frame{\frametitle{\textbf{Clickers}}
\large

You all should have gotten a clicker with your textbook from the bookstore.

\pause
\bigskip
\bigskip
\bigskip

... wait, you didn't?

\begin{columns}

\begin{column}{0.5\textwidth}
\end{column}
\pause
\begin{column}{0.5\textwidth}
...I guess we have some extras...

\pause

\bigskip

\includegraphics[width=0.6\textwidth]{trollface.png}

\pause

\end{column}
\end{columns}
\bigskip
\bigskip
\bigskip


We're using an extremely high-tech, state-of-the-art clicker system in this class.
Make sure you get one, and bring it to class every day.
}

\frame{\frametitle{\textbf{Clickers}}
\Large
\BC
If you ever forget your card, click on the ``Forgot your response cards?'' link.

\BS\BS\BS

You can then find the image that you want and \\hold up your cellphone/tablet/laptop in class.

\BS\BS\BS


\EC
}



\frame{\frametitle{\textbf{Let's practice: Who are you?}}
\Large
What academic year are you?
\bigskip

\Huge

\color{A}A: Freshman \\
\color{B}B: Sophomore \\
\color{C}C: Junior \\
\color{D}D: Senior \\
\color{E}E: Graduate student / non-degree

}

\frame{\frametitle{\textbf{Who are you?}}
\Large
What's your primary field of study?

\bigskip

\Huge

\color{A}A: Science / engineering \\
\color{B}B: Social sciences / Maxwell / international relations\\
\color{C}C: Management / business / marketing / accounting\\
\color{D}D: SUNY ESF \\
\color{E}E: None of these

\Large

\bigskip
\bigskip
}

\frame{\frametitle{\textbf{Who are you?}}
\Large
What's your primary field of study?

\bigskip
\Huge

\color{A}A: Visual / performing arts \\ 
\color{B}B: Liberal arts / humanities\\
\color{C}C: Communication / Newhouse \\
\color{D}D: The iSchool \\ 
\color{E}E: Something else 

}

\frame{\frametitle{\textbf{Who are you?}}
\Large
What's your primary field of study?

\bigskip
\Huge

\color{white}A: Visual / performing arts \\ 
\color{white}B: Liberal arts / humanities \\
\color{white}C: Communication / Newhouse \\
\color{blue}D: The iSchool \\ 
\color{blue}E: Something else 

}

\frame{\frametitle{\textbf{Someone threw a gauntlet at you...}}

\large
``With more knowledge comes deeper, more wonderful mystery... 
with pleasure and confidence we turn over each new stone to find 
unimagined strangeness leading on to more wonderful questions and mysteries--certainly
a grand adventure!

\bigskip

Our poets 
do not write about [this]; our artists do not try to portray [it]. I don't know why. 
{\bf Is nobody inspired by our present picture of the universe?} [Science] remains unsung
by singers, so you are reduced to hearing not a song or poem, but an evening lecture about it.
Is no one inspired by our present picture of the universe? {\bf This is not yet a 
scientific age.}''

\bigskip
\bigskip

\begin{flushright}--Richard Feynman, from {\it The Value of Science} (1955)
\end{flushright}
}

\frame{\frametitle{\textbf{Do you agree with Feynman?}}
\Large
... are we not yet living in a ``scientific age''? Were we in 1955?

\bigskip


\color{A}A: No, we don't live in a scientific age \\ 
\color{B}B: We didn't then, but we do now \\
\color{C}C: We did then, and we still do! \\
\color{D}D: What a silly question! 

\pause

My answer comes from a humble state park in Oregon...
}

%\frame{\frametitle{\textbf{Our poets and writers have (always) been busy...}}
%\includegraphics[width=1.0\textwidth]{collage.jpg}
%}

\frame{\frametitle{\textbf{The eclipse of 2017}}
	
	\BC
	\includegraphics[width=0.8\textwidth]{lisk-corona.jpg}
	\small
	
	\it(by Keith Lisk, shot in Paducah, KY: \url{https://www.dpreview.com/forums/post/60016117})
	\EC
}
\frame{\frametitle{\textbf{The eclipse of 2017}}
	
	\BC
	\includegraphics[width=0.8\textwidth]{viralclimber1.jpeg}
	\tiny
	
	\it(photographers Andrew Studer and Ted Hesser; climbers Tommy Smith and Martina Tibell; Smith Rock State Park, OR)
	\EC
	\small
	\BS
	\url{https://www.dpreview.com/articles/5501288570/how-the-viral-climber-eclipse-photo-and-video-were-shot}
	
	\url{https://youtu.be/H6rxml7TRf0}
}




\frame{\frametitle{\textbf {Optimism and humanism}}
	
	\Large
	
	Our society is constantly telling tales about and finding beauty in the heavens.
	
	\BS\BS\pause
	
	... but we have been {\it living} the greatest story in history, and we don't tell it enough:\\ the story of humanity's burgeoning knowledge and capacity over the last millennia.
	
	\BS\BS\pause
	
	... how we learned about the workings of the Universe ...
	
	\BS\pause
	
	... how we harnessed that knowledge to do fantastic things ...
	
	\BS\pause
	
	... how we grew in knowledge about our place in the Universe ...
	
	\BS\pause
	
	... and how, when that knowledge is humbling, we can take that humility to heart without fear or shame and use it, too, to further our cause.
	\BS\BS\pause
	
	{\bf This course is an attempt to tell that story}. I hope you find it as glorious as I do.
}

%\frame{\frametitle{\textbf{Astronomy's broader impact on human thought}}
%\Large
%Astronomy has been inspiring art, philosophy, music, and literature since those things
%have existed. 
%
%\bigskip
%\bigskip
%
%Why is the fourth planet named Mars?
%
%\bigskip
%\bigskip
%\bigskip
%
%We're going to spend much of our class looking up ... but we'll spend a bit of time
%looking back down, too, to see how astronomy has influenced so many fields of human endeavor.
%}

\frame{\frametitle{\textbf{Course organization: four units}}
\Large
\begin{enumerate}
\item{The motions of the sky}
\item{Astromechanics}
\item{The science of light}
\item{Humanity and the cosmos}
\end{enumerate}

\color{B}

\BS

The first three of these will be split into two halves so you never have a quiz on too many things at a time.
}

\frame{\frametitle{\textbf{Naked-eye astronomy}}
\Large
\begin{columns}
  \begin{column}{0.5\textwidth}
  \BI
    \item{What can we see from Earth?}
    \medskip
    \item{What changes do we see in the sky?}
    \medskip
    \item{How are they explained by Earth's motion?}
    \medskip
    \item{What causes the phases of the Moon and the changing seasons?}
    \medskip
    \item{What about the planets?}
  \EI
  

  
  \end{column}
  \begin{column}{0.5\textwidth}
  \includegraphics[width=1.0\textwidth]{alma-star-trails.jpg}
\end{column}
\end{columns}
}

\frame{\frametitle{\textbf{Astromechanics}}
\Large
\begin{columns}
  \begin{column}{0.5\textwidth}
  \BI
  \color{A}
\item{How do we know the planets orbit the Sun?} 
\medskip
\item{What do the motions of the planets really look like?}
\medskip
\item{How do the laws of physics cause them to move this way?}
\medskip\pause
\color{B}
\item What is science?
\medskip\pause
\item What is pseudoscience?
  \EI
  \end{column}
  \begin{column}{0.5\textwidth}
  \includegraphics[width=1.0\textwidth]{kep6.png}
\end{column}
\end{columns}
}

\frame{\frametitle{\textbf{Light and the electromagnetic spectrum}}
\Large
\begin{columns}
  \begin{column}{0.5\textwidth}
  \BI
  \color{A}
\item{What is light?}
\medskip
\item{What kind of light do hot objects produce?}
\medskip
\color{B}
\item{How do different materials and chemicals interact with light?}
\medskip
\color{E}
\item{How do we use these properties to study things in the sky?}
\medskip
\item{What has this taught us about the Sun?}
  \EI
  \end{column}
  \begin{column}{0.5\textwidth}
  \includegraphics[width=1.0\textwidth]{blackbody-spectrum.png}
  \end{column}
\end{columns}
}

\frame{\frametitle{\textbf{Humanity and the cosmos}}
\BCC
\HC
  \BI
  \color{A}
  \item How and when did the Solar System form?
  \item Why are the different planets different?
  \item What determines Earth's climate, and how are we affecting it?
  \EI
  \HC
  \BI
  \color{B}
  
\item{What are the past and present of spaceflight?}
\item{... what might its future be in our lifetimes and beyond...}
\item{... and where else in the Universe might we find life, and what might it look like?}
  \EI
  \ECC
\begin{center}  \includegraphics[width=0.7\textwidth]{moonshorty_apollo17_1498.jpg}
\end{center}
}

\frame{\frametitle{\textbf{Course components: the labs}}
\Large
\BI
\item{Labs start on the {\bf second week of class}.}
\item{ {\bf Prelabs:} Every lab has a prelab. You {\it must} complete the prelab and bring it with you to lab, or you won't be able to do that lab.}
\item The prelab for the first lab will be available Thursday.
\item{Take-home labs assigned later in September}
\item{Labs meet in Holden Observatory, led by TA's}
\EI
}

\frame{
	\Large
	
	How likely is it that you're going to get the exams for this class, look at them, and say ``Oh! I know the answer to this question!'' and write it down?

\BS\BS\BS


\BCC

\HC

	
	\color{A} A: Not very likely \\
	\color{B} B: Only sometimes \\
	\color{C} C: Most of the time \\
	\color{D} D: All the time! \\ \pause
	\color{E} E: ... okay, what's the deal here?

\BS\pause



\HC

\color{Silver}

Science isn't mostly about {\it knowing things}. 

\BS\BS

It's about:

\BI
\item talking about things
\item figuring out things
\item doing things
\EI
\BS\BS\BS


All of these are skills you have to {\it practice}. How do we do that?
\ECC	
}

\frame{\frametitle{\textbf{Course components: in-class tutorials}}
	
	\Large
	
	To get good at figuring out stuff, you have to practice figuring out stuff.
	\BS\BS
	
	We'll spend a lot of time in class working through exercises designed to get you this practice.
	
	\BS\BS
	
	Take them seriously; they are the best preparation for the exams that you can have.
	
	\BS\BS
	
	Some of the tutorials will relate to a short homework set. You won't turn these in, but quizzes/exams will have things very similar to the homework on them;
	if you have done the homework, these questions should be freebies.
	
	}


\frame{\frametitle{\textbf{Course components: writing and thinking and creating}}
\large

What kind of homework/classwork do you have in your philosophy class?

\pause\bigskip

... you're going to be writing a paper and doing a final creative project (which might be another paper),
in this class, too. These papers and project will focus on connections between the science of astronomy and 
other disciplines in the humanities.

}


\frame{\frametitle{\textbf{Course components: weekly ``checkins''}}
	\large
	
	Most weeks, I'll send you a link to a Google form on Friday. (This week, it'll be early -- today!)
	
	\BS
	
	I'll ask two kinds of questions:
	
	\BI
	\item Things to see how you're doing, in general
	\item Specific questions about your ideas about the following week's material
	\EI
	
	\BS\BS
	
	I'll also give you space to ask me things, which I'll answer in a FAQ page or by personal email if you ask me something personal.
	
}


\frame{\frametitle{\textbf{I'm here to help you!}}
\Large
My full-time job is to help you all (and my other students). This is your class, not mine.

\bigskip
This means:
\BI
\large
\item{Interrupt me any time in class if you have a question}
\item{Yell at me if you have a question and I don't see your hand}
%\item Send a message on Discord (we watch this during class too)
\item{Come to office hours: Monday 10 AM-noon and Wednesday 2-4 PM in the Physics Clinic (room 112)}
\item{Come bang on my office door (room 215) -- I'll often be around}
\item{Email me and ask for help: wafreema@syr.edu}
\item{If you have questions you'd like addressed (``ask the physicist!''), or course suggestions, please send them to me by email or on the weekly checkins (and get extra credit, if they're good!)}
\EI

}

\frame{\frametitle{\textbf{The cosmic perspective: measuring distance}}
\Large
\begin{center} \color{Red}``Baltimore is about five hours away.''\end{center}

Does this statement make sense as a way to describe the distance to Baltimore?

\bigskip
\bigskip

\color{A}A: Yes \\ 
\color{B}B: No  \\
\color{C}C: Yes, if I give you some other information...

\bigskip
\bigskip

}

\frame{\frametitle{\textbf{The cosmic perspective: measuring distance}}
\Large
\begin{center} \color{Red}``Beijing is about 1/15 of a second away.''\end{center}

\pause

\normalsize (This is something anyone who's tried to play a video game with someone 
across the ocean knows about)

\bigskip
\bigskip

\large
\pause
$\rightarrow$ We can measure long distances by {\it how long it takes light to get there!}

\BS

... if Beijing is one-fifteenth of a light-second away, then a {\it light-year} has to be
a pretty long way...
}

\frame{\frametitle{\textbf{Three measures of distance...}}
\Large
Inside the Solar System, it's also useful to measure distances with a different yardstick:
the distance to the Sun. This is called an {\color{Red}astronomical unit}, or AU.

\bigskip

We have:

\bigskip

\begin{itemize}
\item{1 kilometer}
\BI
\item{(good for measuring Earth-size things)}
\EI
\item{1 AU = 150 million km ($1.5 \times 10^8$ km) = about 9 light-minutes}
\BI
\item{(good for measuring distances to the planets)}
\EI
\item{1 light-year = 60,000 AU = 9 trillion km ($9 \times 10^{12}$ km)}
\pause
\item{1 universe = 14 billion light-years!}
\end{itemize}
}

\frame{\frametitle{\textbf {Takeaways: size and scale}}

\large

\BI
\item Inner planets like Earth are about 10,000 km across
\item The Moon is around 400,000 km from Earth
\item The inner planets are hundreds of millions of km -- around an AU -- away from the Sun
\item Outer planets like Jupiter are about 100,000 km across and tens of AU from the Sun
\item The nearest star is around 250,000 AU (4 ly) from us
\EI

\BS
\BS

We tend to usually use AU to measure things in this class, since we are mostly concerned with the Solar System.

\BS

We don't expect you to memorize exact numbers, but you should have a general idea of the scale of things near us in the Universe.
}

\frame{\frametitle{\textbf{Another Freeman can explain it better than me!}}
\Large
\centerline{\url{https://www.youtube.com/watch?v=44cv416bKP4}}

\pause

\bigskip
\bigskip
\bigskip
\bigskip

\centerline{... where are we in all this?}
}

\frame{\frametitle{\textbf{Sizes of things}}
	
	\Large
	
	\BCC
	\column{0.25\textwidth}

	\column{0.25\textwidth}
	\bf Meters
	\column{0.25\textwidth}
	\bf AU
    \column{0.25\textwidth}
    \bf Light travel time
    \ECC
     \color{E}
    \pause\BCC
    \column{0.25\textwidth}
    A child
    \column{0.25\textwidth}
    1 m
    \column{0.25\textwidth}
    
    \column{0.25\textwidth}
    3 nanoseconds
    \ECC
    \color{A}
   	\pause\BCC
    \column{0.25\textwidth}
    Size of Earth
    \column{0.25\textwidth}
    12000 km
    \column{0.25\textwidth}
    80 millionths
    \column{0.25\textwidth}
    40 milliseconds
    \ECC
    \color{B}
   	\pause\BCC
    \column{0.25\textwidth}
    Distance to Moon
    \column{0.25\textwidth}
    400,000 km
    \column{0.25\textwidth}
    1/400 AU
    \column{0.25\textwidth}
    1 second
    \ECC
        \color{C}
    \pause\BCC
    \column{0.25\textwidth}
    Distance to Sun
    \column{0.25\textwidth}
    150 million km
    \column{0.25\textwidth}
    1 AU
    \column{0.25\textwidth}
    10 minutes
    \ECC
        \color{D}

    \pause\BCC
    \column{0.25\textwidth}
    Jupiter's orbit
    \column{0.25\textwidth}
    1 billion km
    \column{0.25\textwidth}
    5 AU
    \column{0.25\textwidth}
    1 hour
    \ECC
    \color{E}
    \pause\BCC
    \column{0.25\textwidth}
    Neptune's orbit
    \column{0.25\textwidth}
    5 billion km
    \column{0.25\textwidth}
    30 AU
    \column{0.25\textwidth}
    5 hours
    \ECC
        \color{A}
    \pause\BCC
    \column{0.25\textwidth}
    Furthest spacecraft
    \column{0.25\textwidth}
    20 billion km
    \column{0.25\textwidth}
    125 AU
    \column{0.25\textwidth}
    1 day
    \ECC
    \color{B}
    \pause\BCC
\column{0.25\textwidth}
Nearest other star
\column{0.25\textwidth}
40 trillion km
\column{0.25\textwidth}
300,000 AU
\column{0.25\textwidth}
4 years
\ECC
\BS\pause
\color{Silver}
    \pause\BCC
\column{0.25\textwidth}
The Milky Way
\column{0.25\textwidth}

\column{0.25\textwidth}
600 million AU
\column{0.25\textwidth}
100,000 years
\ECC
\BS
\color{Silver}
\pause\BCC
\column{0.25\textwidth}
The Universe
\column{0.25\textwidth}

\column{0.25\textwidth}

\column{0.25\textwidth}
14 billion years
\ECC

    
}


\frame{\frametitle{\textbf{Physics: Everything here is all the same!}}
\Large
\centerline{It's no accident this class is in the physics building!}

\bigskip
\bigskip

\large
Everything in the Universe is made of the same sort of stuff.

\bigskip
\bigskip

\BI
\item{Those distant billions of galaxies, and their billions of stars each...}
\item{... the planets that we now know orbit many of those stars...}
\item{... the atoms that make up our own sun...}
\item{... the matter here on Earth...}
\item{... and even the atoms that make up you and I...}
\EI

... are all made of the same sort of matter, doing the same dance they've been doing 
since the beginning.  
}

\frame{\frametitle{\textbf{By studying a few dancers, we learn about them all..}}
\Large
\centerline{\url{https://youtu.be/W-csPZKAQc8}}

\bigskip
\bigskip


\pause

This is a computer simulation of a collision that will happen in a few billion years.

\bigskip
\bigskip

Using a few principles you'll learn about in this class, and a computer, you can make this! 

\bigskip
\bigskip

We, on our little rock, can actually {\it understand} how this all works!
}

\frame{\frametitle{\textbf{Next time: the night sky}}
\Large
Thursday: How does the night sky move each night?

\bigskip

{\bf Stuff to do:}

\bigskip

\BI
\item{Go find the course website and read the syllabus}
\item Answer the survey I'll send out tonight
%\item Join the course Discord if you want
%\item Blow up your Earths
\pause
\item Get to know the folks around you!
\item Read the first bits of your textbook (see the calendar)
\EI
}

%\frame{\frametitle{\textbf{Summary}}
%\large
%\begin{center}
%The British have a long, dignified tradition of astronomy. 
%
%\pause
%\bigskip
%\bigskip
%
%The British also have a long, dignified tradition of music.
%
%\pause
%\bigskip
%\bigskip
%
%When they mix them, though, the dignity leaks out a bit...
%\pause
%\url{https://www.youtube.com/watch?v=buqtdpuZxvk}
%\end{center}
%}
\end{document}


