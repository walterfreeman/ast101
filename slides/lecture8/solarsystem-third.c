...
int main(int argc, char **argv)
{
  ...
  ss[2].m=0.0000;
  ss[2].r=xhat*1.666;
  ss[2].v=sqrt(G/ss[2].r.x)*yhat*0.957;
  ...
  ss[3].m=0.0000;
  ss[3].r=xhat*0.466;
  ss[3].v=sqrt(G/ss[3].r.x)*yhat * 0.895;
  ...
  ss[4].m=0.0000;
  ss[4].r=xhat*0.728;
  ss[4].v=sqrt(G/ss[4].r.x)*yhat*0.997;
  ...
  ss[5].m=0.0000;
  ss[5].r=xhat*5.45;
  ss[5].v=sqrt(G/ss[5].r.x)*yhat*0.97;
  ...
  ss[6].m=0.0000;
  ss[6].r=xhat*10.1;
  ss[6].v=sqrt(G/ss[6].r.x)*yhat*0.969;
  ...
  ss[7].m=0.0000;
  ss[7].r=xhat*15;
  ss[7].v=sqrt(G/ss[7].r.x)*yhat*0.25+sqrt(G/ss[7].r.x)*zhat*0.1;
...
double period[nplanets]; for (int i=0;i<nplanets;i++) period[i]=-1;
 ...
 for (int i=0;i<nplanets;i++)
  ...
  for (double t=0;1;t+=dt)
  {
    for (int i=0;i<nplanets;i++)
    {
       ss[i].r = ss[i].r + ss[i].v * dt*0; // position update
    }
    ...
    for (int i=0;i<nplanets;i++)
    {
      vector vnew = ss[i].r + ss[i].v * dt*1; // position update
      if ((vnew.y<0 && ss[i].r.y>0) && ph[i]==0) {ph[i]=hypot(ss[i].r.x,ss[i].r.y);}
      
      if ((vnew.y>0 && ss[i].r.y<0) && period[i]==-1) {period[i]=t; printf("!Planet %d has an orbital period of %.2f years: aphelion %.2f, perihelion %.2f\n",i,period[i],hypot(ss[i].r.x,ss[i].r.y),ph[i]);}
      ss[i].r=vnew;
    }
...
//     printf("!Assigning location %e,%e,%e to ringbuffer site %d\n",ss[i].r.x,ss[i].r.y,ss[i].r.z,(rbl)%ringbuf_size);
     ...
     if (period[i] > 0)
     {
       ...
       if (tfrac > 0.45 && tfrac < 0.55) connect[i][rbl%ringbuf_size]=1;
     }
   }
   }     
*/
    ...
    for (int i=0;i<nplanets;i++)
    {
      ...
      printf("ct3 %d %e %e %e %e\n",i,ss[i].r.x-ss[1].r.x*fact, ss[i].r.y-ss[1].r.y*fact, ss[i].r.z-ss[1].r.z*fact,ss[i].size);
      if (i == 0) printf("ulc3 %e %e %e %e\n",ss[i].r.x-ss[1].r.x*fact, ss[i].r.y-ss[1].r.y*fact, ss[i].r.z-ss[1].r.z*fact,ss[i].size);

      printf("C %e %e %e\n", ss[i].color.x/3, ss[i].color.y/3, ss[i].color.z/3);

     {
     }
    }

    printf("lightfrom %e %e %e\n",-ss[1].r.x*fact,-ss[1].r.y*fact,-ss[1].r.z*fact);    
    printf("F\n");
    tup += tskip;
  }
  }
