...
void idle(void)
{
  ...
  for (double x=-0.9; x<0.9; x+=1.8/window_size)
  {
   ...
   glEnd();
   glColor4f(1,1,1,0.6);
    for (double l=lmin; l<lmax; l+=(lmax-lmin)/(window_size*5))
    {
      double x=getx(l), y=ampl(l,t)/dmaxamp-0.8;
     circle(x,y,0.01,5);
    }      
  ...
  char string[80];
  snprintf(string, 80, "%f pixels per nm",window_size_x/(lmax-lmin));
  renderBitmapString(-0.9, -0.9, 0, GLUT_BITMAP_HELVETICA_18,string);
  ...
  glEnd();
    lastx=-10;
  for (double l=100; l<10000; l+=100)
  {
  ...
  glEnd();
    ...
    {
    lastx=x;
    renderBitmapString(x-0.02,-0.90,0,GLUT_BITMAP_HELVETICA_18,thestring);
    }
  } 
  lastx=-10;
  for (double e=100; e>=1; e-=1)
  {
    x=getx(1240/e);
    if (fabs(x)>0.9) continue;
  glBegin(GL_LINES);
    ...
    glVertex3f(x,0.8,0);
  glEnd();
    sprintf(thestring,"%d",(int)e);
  glColor4f(1,1,1,1);
    if (x-lastx > 0.2)
    {
    lastx=x;
    renderBitmapString(x-0.006,0.87,0,GLUT_BITMAP_HELVETICA_18,thestring);
    }
  } 
  sprintf(thestring,"Energy per photon (eV)");
  renderBitmapString(-0.2,0.92,0,GLUT_BITMAP_HELVETICA_18,thestring);
  sprintf(thestring,"Wavelength (nanometers)");
  renderBitmapString(-0.2,-0.95,0,GLUT_BITMAP_HELVETICA_18,thestring);
  ...
  if (autoscale)
  {sprintf(thestring,"Automatic scaling on");
  renderBitmapString(-0.5,0.4,0,GLUT_BITMAP_HELVETICA_18,thestring);
  }
   glutSwapBuffers();
