...
\frame{\frametitle{\textbf{Announcements: change in course format}}
\large

As you know, we have had to seriously curtail the amount of printing we do for your class. 

\BS

This will impact the way we do homework, lecture, labs, and possibly even quizzes.


\BS\BS

For {\bf \color{Red}lab}:

\normalsize

\BI
\item You should bring {\bf \color{Red} paper} and a laptop or tablet to lab (if you have one)
\item You can access the lab materials electronically or print copies for yourself
\item You will need paper to keep notes
\item You'll get a grade based on a conversation with the lab TA while you are at lab
\item You won't need to turn anything in or do anything after your lab
\EI
}
...
\frame{\frametitle{\textbf{Announcements: change in course format}}
	...
	For {\bf \color{Red}homework}, we will likely post questions electronically (as a PDF); you will answer them on your own paper and turn them in as before.
	
	\BS\BS\BS\BS
	
	We will replace the {\bf \color{Green} in-class tutorials} with lots of {\color{A}A}/{\color{B}B}/{\color{C}C}/{\color{D}D} questions plus tutorials that I will lead (like we did for the first part of the energy exercises).
	
	\BS\BS\BS\BS
	
	{\color{C} \bf Quizzes} may continue as we have been doing them if we can print that many sheets. We may also switch to Blackboard quizzes that you will do in the auditorium. We'll let you know well in advance of the next quiz.
}

\frame{\frametitle{\textbf{Announcements: papers}}
	\large
	
	Remember that your first paper is due today unless you have asked for and gotten an extension.
	
	\BS
	
	Turn in a paper copy to your TA's mailbox {\it and} email a copy to {\tt suast101projects@gmail.com}. The emailed copy is for our enduring record; the paper copy is what we will grade.
}
...
\frame{\frametitle{\textbf{Properties of science: a reminder}}
	\large
...
\frame{\frametitle{\textbf{Properties of science, in our story}}
	...
	\color{A}
	\item {\bf Empiricism:} the ultimate authority is what we measure about the world around us, not what we think. 
	\item \normalsize The new scientific approach to mechanics started with Kepler, Sophie, and Tycho's observations of the planets, observations made by Galileo and Newton, and then built from there
	...
	\color{B}
	\item \large {\bf Self-skepticism:} someone making a scientific claim should actively search for things that might prove themselves wrong 
	\item \normalsize Kepler was convinced planetary orbits were circles and tried {\it very hard} to make circular orbits work...
	until the data convinced him otherwise
	\item \normalsize Tycho did {\it not} do this: when he observed a lack of stellar parallax, he didn't consider the possibility that the stars might just be very far (250,000 AU) away
	\EI
...
\BI
	...
	\item \normalsize Just because Galileo threatened a dominant religious/political paradigm doesn't make him wrong!
	\item \normalsize Understanding the sky from many different places on Earth is critical to knowing how it works
	\EI
}
...
\frame{\frametitle{\textbf{Principles that come from these:}}
	...
	\item Particle physicists and their ``one in a million'' rule
	\BS
	\pause
...
\frame{\frametitle{\textbf{Two notes}}
	...
	about math errors or physics mistakes here. But a sound process of science {\color{D}catches its own mistakes} through self-skepticism and transparency.
...
\frame{\frametitle{\textbf{Cherry-picking and biased data}}
	...
	Example: the ``fifth force'', a proposed modification to $F_g = \frac{GMm}{r^2}$
...
\frame{\frametitle{\textbf{Other sorts of biased data}}
	
	\large
	
	\begin{minipage}{0.5\textwidth}
	
	The US Army Air Forces bombed Nazi Germany using large aircraft during World War II.
	
	\bigskip
	
	Many aircraft were hit by enemy fire but returned to airfields in England.
	
\end{minipage}
\begin{minipage}{0.49\textwidth}
	\begin{center}
		\includegraphics[width=0.8\textwidth]{b17-collision.jpg}
		
		\scriptsize
		\it A B-17 ``Flying Fortress'' that nearly lost its tail after a collision with a German fighter. This aircraft, still able to fly, returned to an Allied airfield in Algeria with no crew injured, including the tail gunner! 
	\end{center}
\end{minipage}
	...
	The USAAF did an assessment of these aircraft to see where they were being hit: on the fuselage, on the wings, on the tail, but not in the engines or the nose.
	
	\bigskip
	
	Should the USAAF add extra armor to...
	
	\begin{itemize}
		\item \color{A}A: The locations where the returning aircraft had been hit
		\item \color{B}B: The locations where the returning aircraft had {\it not} been hit
		\item \color{C}C: All locations on the aircraft equally?
	\end{itemize}
