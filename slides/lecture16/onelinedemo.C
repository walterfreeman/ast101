...
float init_bessel(float x, float y,int m, int n)
{
  float r=hypot(x,y);
  if (r>1) return 0;
...
int main(int argc, char **argv)
{
  if (argc < 6)
  {
    printf("!Usage: <this> [b <angular mode number> <radial mode number>] or [t <center> <radius> <amplitude>] N dt\n");
    exit(0);
  } 
  char mode=argv[1][0];
  int m,n;
  double center,rad,str,dt;
  if (mode == 'b')
  {
    m=atoi(argv[2]);
    n=atoi(argv[3]);
  ...
  for (t;1;)
  {
    ...
    if (hypot(x,y)>1+dx*3) continue;
          printf("C %e %e %e\n",0.5+z[(i)*(N+1)+j]*5,0.5,0.5-z[(i)*(N+1)+j]*5);
	  if (hypot(x,y)<1+dx*1.1 && hypot(x+dx,y)< 1+dx*1.1 && i<N) printf("l3 %e %e %e %e %e %e\n",x,y,z[(i)*(N+1)+j],x+dx,y,z[(i+1)*(N+1)+j]);
	  if (hypot(x,y)<1+dx*1.1 && hypot(x,y+dx)< 1+dx*1.1 && j<N) printf("l3 %e %e %e %e %e %e\n",x,y,z[(i)*(N+1)+j],x,y+dx,z[(i)*(N+1)+j+1]);
	}
      printf("F\n");
    }
  }
