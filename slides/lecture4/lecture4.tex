...
\begin{center}
	I'm catching up answering my email; I hope to be caught up Friday.
...
\item If it's for a {\it compelling reason} (illness, emergency, academic/professional conflict, family commitment), you may do this as you need.

\item You may do this for any other reason {\color{B}once} per semester.
...
\item You do {\it not} need to tell me unless I need to do something.

\EI
}

\frame{\frametitle{\textbf{Announcements: Homework quiz 1}}
	
	\Large 
	
	\begin{center}
		Your first quiz is next Tuesday (not today), during the last portion of class. It will have three questions, as described on the homework.
		
		\BS\BS
		
		If you have a CDR accommodation for extra time or a lower-distraction environment, we can provide that for you in the physics building. 
		
		\BS\BS
		
		You may go to CDR's facility too if you choose.
...
\Large
Questions about the exercises or homework? Want to study with me? Come chat!

\bigskip
\bigskip
\bigskip

This is the absolute very best thing you can do to study for our class.
...
I have office hours Wednesday 2-4 or Monday 10-12; come see me in room 215!
}
...
\bigskip
\color{C}C:The Sun is different from the other stars and we can't use the same logic to understand how it works \\
\bigskip
...
\frame{
	...
	\color{A}A: The Sun is close enough that the Earth's movement matters, unlike for other stars \\
	{\color{E} $\rightarrow$ This is the modern perspective: the Earth's movement carries it to the other side of the Sun during a year.}
	
	\bigskip
	\color{B}B: The Sun lies on a different celestial sphere than the stars, which 
	turns at a different rate \\
	
		{\color{E} $\rightarrow$ This was the ancient perspective: they imagined that the Sun's rotated just a little bit slower.}
	
	
	\bigskip
	\color{C}C: The Sun is different from the other stars and we can't use the same logic to understand how it works\\
	 
	{\color{E} $\rightarrow$ Since the Sun is like all the other stars but just closer, we should be able to understand it in the same way.}
	 
	\bigskip
	\color{D}D: The Sun is close enough that we notice its movement, unlike the other stars
	
	{\color{E} $\rightarrow$ This isn't correct -- the Sun doesn't move much, just like the other stars don't move much, over one year.}
...
\frame{\frametitle{\textbf{Understanding this}}
	
	\Large
	
	We need two pieces to understand this fully. 
	
	\BS\BS
	
	\begin{enumerate}
		\color{A}
		\item First, we need to understand what the sky looks like as Earth moves around the Sun. This is your exercise.
		
		\BS\BS
		\color{B}
		\item Then, we need to return to this question of ``what happens in a day?''.
	\end{enumerate}
}
...
\frame{
\Huge
\BC Work through the exercise for today. Your next homework is
on the back. \EC

\bigskip
\BC \large We will talk about astrology and timekeeping after this.
\EC
}
...
\frame{\frametitle{\textbf{The Zodiac and the astrological calendar}}
	...
	How do you know when it is September?
