...
int main(int argc, char **argv)
{
  ...
  if (argc <= 1)
  {
    fprintf(stderr, "Usage: <this> <sec per day> <sec per month> <sec per year> <axial tilt> <phase angle (0=Yule 180=Lithe)> <observer latitude>\n[sun radius] [earth radius] [moon radius] [moon orbit radius] [earth orbit radius]\n"); 
    exit(0);
  }
  ...
  if (argc > 7) sun_radius = atof(argv[7]);
  if (argc > 8) earth_radius = atof(argv[8]);
  if (argc > 9) moon_radius = atof(argv[9]);
  if (argc > 10) moon_orbit_radius = atof(argv[10]);
  if (argc > 11) earth_orbit_radius = atof(argv[11]);
...
// draw Sun
    ...
    // draw Earth
    lit_globe(earth - earth, rotax*earth_radius, earth_or, rearth,gearth,bearth, axialtilt);
    printf("F\n");
...
// draw rotation axis
    if (axis) arrow(earth+rotax*earth_radius - earth,rotax*3*earth_radius,0,1,0);
