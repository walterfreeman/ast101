...
//       if (connect[i][k%ringbuf_size]) {printf("C %e %e %e\n",0.5+0.5*sin(connect[i][k%ringbuf_size]),0.5+0.5*sin(connect[i][k%ringbuf_size]+1),0.5+0.5*sin(connect[i+2][k%ringbuf_size])); lin(ringbuf[i][k%ringbuf_size],ss[0].r); printf("C %e %e %e\n",ss[i].color.x, ss[i].color.y, ss[i].color.z);}
     }
    }
