  ...
  for (double t=0;1;t+=dt)
  {
    ...
    for (int i=0;i<2;i++)
    {
      for (int j=0;j<2;j++)
      {
        if (i==j) continue;
        vector r;
//        printf("!Considering planet %d at %e,%e,%e; it has velocity %e,%e,%e\n",i,ss[i].r.x,ss[i].r.y,ss[i].r.z,ss[i].v.x,ss[i].v.y,ss[i].v.z);
//        printf("!Considering planet %d at %e,%e,%e; it has velocity %e,%e,%e\n",j,ss[j].r.x,ss[j].r.y,ss[j].r.z,ss[j].v.x,ss[j].v.y,ss[j].v.z);
        
        r=ss[i].r-ss[j].r;
        double rn=norm(r);
//        printf("!Separation between planets %d and %d is %e,%e,%e (norm %e)\n",i,j,r.x,r.y,r.z,rn);
...
//       printf("!ringbuffer: %d of %d\n",k,rbl+ringbuf_size);
       ...
       if (connect[i][k%ringbuf_size]) {printf("C 0 0.5 0\n"); lin(ringbuf[i][k%ringbuf_size],ss[0].r); printf("C %e %e %e\n",ss[i].color.x, ss[i].color.y, ss[i].color.z);}
     }
    }
    printf("F\n");
    tup += tskip;
